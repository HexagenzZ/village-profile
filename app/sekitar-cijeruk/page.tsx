...
import { Star, MapPin, Compass, ArrowLeft } from "lucide-react";
import Link from "next/link";

export default function SekitarCijerukPage() {
  const [selectedDestination, setSelectedDestination] = useState<DestinationItem | null>(null);

  const sekitarList = useMemo(() => {
    return DESTINATIONS.filter((d) => d.category === "sekitar");
  }, []);
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="border-b border-stone-200 pb-6">
          ...
          <p className="text-sm text-stone-600 max-w-3xl mt-1.5 leading-relaxed">
            Halaman ini memuat destinasi unggulan di wilayah tetangga yang berdekatan dengan Desa Cijeruk (Cigombong, Caringin, Tamansari, dan Danau Lido) untuk melengkapi rencana perjalanan liburan Anda.
          </p>
        </div>

        {/* Grid List */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 pt-4">
          {sekitarList.map((item) => (
            <div
              key={item.id}
