...
import { Star, MapPin, UtensilsCrossed, ArrowLeft } from "lucide-react";
...
export default function KulinerPage() {
  ...
  const kulinerList = useMemo(() => {
    return DESTINATIONS.filter((d) => d.category === "kuliner");
  }, []);

  const subCategories = useMemo(() => {
    const subs = Array.from(new Set(kulinerList.map((d) => d.subCategory)));
    return ["all", ...subs];
  }, [kulinerList]);

  const filtered = useMemo(() => {
    if (selectedSubCategory === "all") return kulinerList;
    return kulinerList.filter((d) => d.subCategory === selectedSubCategory);
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 border-b border-stone-200 pb-6">
          <div>
            ...
            <p className="text-sm text-stone-600 max-w-2xl mt-1.5">
              Cita rasa autentik Sunda, saung lesehan liwet di atas kolam, hingga kafe estetik berpanorama Gunung Salak dengan sajian kopi lokal Cijeruk.
            </p>
          </div>
